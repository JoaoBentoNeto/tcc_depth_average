...
def calculate_simulation_depths(geometry_mask: np.ndarray) -> np.ndarray:
    ...
    min_width = round(np.min(active_nodes) * 2)
    max_width = round(np.max(active_nodes) * 2)

    start_depth = min_width / 2.0
    end_depth = max_width * 2.0

    depth_values = np.round(np.linspace(start_depth, end_depth, 7)).astype(int)

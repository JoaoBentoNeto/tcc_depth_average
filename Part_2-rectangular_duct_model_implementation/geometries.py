...
def create_duct(aspect_ratio: float, height: float, resolution: float, lattice_length: int) -> np.ndarray:
    ...
    print(
        f"Rectangular duct maximum channel width = {round(np.max(semi_width_map) * 2)} voxels = {round(np.max(semi_width_map) * 2) * resolution}  \u03bcm"
    )
    print(
        f"Rectangular duct minimum channel width = {round(np.min(semi_width_map) * 2)} voxels = {round(np.min(semi_width_map) * 2) * resolution}  \u03bcm"
    )
    print(
        f"Rectangular duct mean channel width = {round(np.mean(semi_width_map) * 2)} voxels = {round(np.mean(semi_width_map) * 2) * resolution}  \u03bcm\n"
    )
...
def create_unit_cell(
    ...
    solid = center_mask | top_left_mask | top_right_mask | bot_left_mask | bot_right_mask

    domain = (~solid).astype(np.uint8)

    semi_width_map = ps.filters.local_thickness(domain != 0)
    print(f"Unit cell maximum channel width = {round(np.max(semi_width_map) * 2)} voxels")
    print(f"Unit cell minimum channel width = {round(np.min(semi_width_map) * 2)} voxels")
    print(f"Unit cell mean channel width = {round(np.mean(semi_width_map) * 2)} voxels\n")

    return domain.astype(np.float64)


def create_constricted_channel(characteristic_length: float, resolution: float) -> np.ndarray:
    ...
    print(f"Constricted channel maximum channel width = {round(np.max(semi_width_map) * 2)} voxels")
    print(f"Constricted channel minimum channel width = {round(np.min(semi_width_map) * 2)} voxels")
    print(f"Constricted channel mean channel width = {round(np.mean(semi_width_map) * 2)} voxels\n")
...
def create_double_pore(
    ...
    mask_channel = (y >= (ny // 2 - channel_width // 2)) & (y <= (ny // 2 + channel_width // 2))
    mask_large_pore = ((x - cx_pore) ** 2 / pore_rx**2 + (y - cy_pore) ** 2 / pore_ry**2) <= 1
    ...
    mask_solid_obstacle = ((x - cx_obstacle) ** 2 / obstacle_rx**2 + (y - cy_obstacle) ** 2 / obstacle_ry**2) <= 1
    data[mask_solid_obstacle] = 0

    semi_width_map = ps.filters.local_thickness(data != 0)
    print(f"Double pore maximum channel width = {round(np.max(semi_width_map) * 2)} voxels")
    print(f"Double pore minimum channel width = {round(np.min(semi_width_map) * 2)} voxels")
    print(f"Double pore mean channel width = {round(np.mean(semi_width_map) * 2)} voxels\n")
...
def create_u_channel(
    ...
    print(f"U channel maximum channel width = {round(np.max(semi_width_map) * 2)} voxels")
    print(f"U channel minimum channel width = {round(np.min(semi_width_map) * 2)} voxels")
    print(f"U channel mean channel width = {round(np.mean(semi_width_map) * 2)} voxels\n")
...
def create_curved_channel(domain_size: int) -> np.ndarray:
    ...
    Returns:
        ...
        solid boundaries.
    """
    channel_width = 0.1 * domain_size

    inner_radius = domain_size // 2 - 2 * channel_width
    outer_radius = domain_size // 2 + 2 * channel_width
    ...
    mask_channel_1 = (distance_sq_1 >= inner_radius**2) & (distance_sq_1 < outer_radius**2)

    distance_sq_2 = (x - (domain_size - 1)) ** 2 + (y - (domain_size - 1)) ** 2
    mask_channel_2 = (distance_sq_2 >= inner_radius**2) & (distance_sq_2 < outer_radius**2)

    geometry[mask_channel_1 | mask_channel_2] = 1

    semi_width_map = ps.filters.local_thickness(geometry != 0)
    print(f"Curved channel maximum channel width = {round(np.max(semi_width_map) * 2)} voxels")
    print(f"Curved channel minimum channel width = {round(np.min(semi_width_map) * 2)} voxels")
    print(f"Curved channel mean channel width = {round(np.mean(semi_width_map) * 2)} voxels\n")
...
def create_diagonal_square(domain_size: int) -> np.ndarray:
    ...
    print(f"Diagonal square maximum channel width = {round(np.max(semi_width_map) * 2)} voxels")
    print(f"Diagonal square minimum channel width = {round(np.min(semi_width_map) * 2)} voxels")
    print(f"Diagonal square mean channel width = {round(np.mean(semi_width_map) * 2)} voxels\n")
...
def create_step_geometry(domain_size: int) -> np.ndarray:
    ...
    print(f"Step maximum channel width = {round(np.max(semi_width_map) * 2)} voxels")
    print(f"Step minimum channel width = {round(np.min(semi_width_map) * 2)} voxels")
    print(f"Step mean channel width = {round(np.mean(semi_width_map) * 2)} voxels\n")
...
def create_slanted_channel(domain_size: int, width_fraction: float = 0.3) -> np.ndarray:
    ...
    semi_width_map = ps.filters.local_thickness(geometry != 0)
    print(f"Slanted channel maximum channel width = {round(np.max(semi_width_map) * 2)} voxels")
    print(f"Slanted channel minimum channel width = {round(np.min(semi_width_map) * 2)} voxels")
    print(f"Slanted channel mean channel width = {round(np.mean(semi_width_map) * 2)} voxels\n")

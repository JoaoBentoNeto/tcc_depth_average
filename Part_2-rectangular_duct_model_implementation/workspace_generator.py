...
import run_depth_average as run
...
def create_simulation_directory(
    ...
    simulation_type: Literal["full", "2D", "grey", "laleian2015", "rdm"],
) -> tuple[str, str]:
    ...
    Args:
        ...
        simulation_type: Defines the routine and environment to be used.
            Must be exactly "full", "grey", "laleian2015" or "rdm".
...
def create_slurm_script(
    ...
    Args:
        ...
        hardware_type: The identifier for the hardware architecture.
            Must be exactly "k40m", "a100" or "cpu".
        simulation_type: Defines the routine and environment to be used.
            Must be exactly "full","2D","grey", "laleian2015","rdm".
        only_one_domain: If True, restricts the subdomains division to a single
            domain.
    ...
    if simulation_type == "laleian2015" or simulation_type == "rdm":
        body = textwrap.dedent(f"""\
            ...
            python -u /home/joao.neto/TCC/Part_2-rectangular_duct_model_implementation/run_depth_average.py ./{simulation_name}.db
            """)
    else:
...
def depth_average_db(
    local_path: str,
    remote_path: str,
    simulation_name: str,
    simulation_type: Literal["laleian2015", "rdm"],
    ...
    Args:
        ...
        simulation_name: Name of the simulation.
        simulation_type: Defines the drag force to be used.
            Must be exactly "laleian2015" or "rdm".
...
def write_sbatch_header(local_parent_path: str) -> None:
    ...
    for simulation_type in ["full", "2d", "grey", "laleian2015", "all", "rdm"]:
...
def greyscale_workspace(
    ...
    porosity_map.tofile(os.path.join(local_sim_path, f"porosity_map_{simulation_name}.raw"))

    # semi_width_map = run.calculate_semi_width_map(depth_map=depth_map)
    semi_width_map = ps.filters.local_thickness(depth_map != 0)
    # print(
    #     f"{simulation_name} maximum channel width = {np.max(semi_width_map) * 2} voxels = {np.max(semi_width_map) * 2 * resolution}  \u03bcm"
    # )
    # print(
    #     f"{simulation_name} mean channel width = {np.mean(semi_width_map) * 2} voxels = {np.mean(semi_width_map) * 2 * resolution}  \u03bcm\n"
    # )
    plt.imshow(semi_width_map)
    plt.axis("on")
    plt.colorbar()
    plt.savefig(os.path.join(local_sim_path, f"{simulation_name}.png"), dpi=300, bbox_inches="tight", pad_inches=0.1)
    plt.close()
    permeability_map = run.calculate_shear_permeability(
        semi_width_map=semi_width_map[:, np.newaxis, :], depth_map=depth_map
    )
    del semi_width_map
    permeability_map = np.multiply(porosity_map, permeability_map, out=permeability_map, dtype=np.float64)
    permeability_map.tofile(os.path.join(local_sim_path, f"permeability_map_{simulation_name}.raw"))
    del porosity_map
    del permeability_map

    (depth_map != 0).astype(np.uint8).tofile(os.path.join(local_sim_path, f"{simulation_name}.raw"))

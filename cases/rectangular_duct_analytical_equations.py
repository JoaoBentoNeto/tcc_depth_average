...
def calculate_permeability_3d(width: float, height: float) -> float:
...
def calculate_permeability_2p5d(width: float, height: float) -> float:
    ...
    k_2p5d = (height**2 / 12.0) * (
        1.0 - np.tanh(r * width / 2.0) / (r * width / 2.0)
    )

    return k_2p5d


def calculate_analytical_velocity_3d(
    ...
    base_factor = (4.0 * height**2 * body_force) / (
        np.pi**3 * kinematic_viscosity
    )
    ...
    cosh_ratio = (
        np.exp(a - b) * (1.0 + np.exp(-2.0 * a)) / (1.0 + np.exp(-2.0 * b))
    )
...
def calculate_analytical_velocity_map_3d(
    x_coords: np.ndarray | float,
    height: float,
    width: float,
    body_force: float,
    ...
    cs2 = 1.0 / 3.0
    kinematic_viscosity = (tau - 0.5) * cs2
    base_factor = (8.0 * height**2 * body_force) / (
        np.pi**4 * kinematic_viscosity
    )
...
def calculate_analytical_velocity_map_2p5d(
    x_coords: np.ndarray | float,
    height: float,
    width: float,
    body_force: float,
